# -*- coding: utf-8 -*-
"""
Created on Sat Jan  2 17:51:35 2021
...
plt.subplots_adjust(top=0.95, bottom=0,left=0,right=0.5)
ax_img.axis("off")
ax_img.title.set_text("title")

ax_roi = plt.axes([0.5, 0.5, 0.5, 0.45])
ax_roi.axis("off")
ax_roi.title.set_text("title")

ax_checkbox = plt.axes([0.525, 0.3, 0.2, 0.15])
labels = ["is bee", "mostly visible", "has mite"]
check = mpl.widgets.CheckButtons(ax_checkbox, labels)

ax_button_set = plt.axes([0.525, 0.2, 0.2, 0.075])
but_set = mpl.widgets.Button(ax_button_set, "Set to DF")


ax_button_j2_first =    plt.axes([0.525, 0.0, 0.1, 0.075])
ax_button_j2_prev =     plt.axes([0.525+0.125, 0.0, 0.1, 0.075])
ax_button_j2_next =     plt.axes([0.525+0.25, 0.0, 0.1, 0.075])
ax_button_j2_last =     plt.axes([0.525+0.375, 0.0, 0.1, 0.075])

ax_button_j2_select =   plt.axes([0.525+0.25, 0.1, 0.1, 0.075])

but_j2_first =  mpl.widgets.Button(ax_button_j2_first, "First")
but_j2_prev =   mpl.widgets.Button(ax_button_j2_prev, "Prev")
but_j2_next =   mpl.widgets.Button(ax_button_j2_next, "Next")
but_j2_last =   mpl.widgets.Button(ax_button_j2_last, "Last")
but_j2_select = mpl.widgets.Button(ax_button_j2_select, "To (x)")

but_j2_first.on_clicked(on_click_j2_first)
but_j2_prev.on_clicked(on_click_j2_prev)
but_j2_next.on_clicked(on_click_j2_next)
but_j2_last.on_clicked(on_click_j2_last)
but_j2_select.on_clicked(on_click_j2_select)


ax_textbox_j2_select =   plt.axes([0.525+0.125, 0.1, 0.1, 0.075])
text_box_j2_select = mpl.widgets.TextBox(ax_textbox_j2_select, "To","1")
text_box_j2_select.on_submit(txt_submit)

f_text = plt.figtext(0.525+0.25, 0.425, "is bee: X\nmostly visible: X\nhas mite: X", 
...
# df.at[index,'label'] = ifor_val # edit an entry

img_extr_path = path_MAIN + path_imgs + row["img_extr"]
img_roi_path = path_MAIN + path_showroi + row["img_overlay"]

try:
    img_extr = plt.imread(img_extr_path)
    img_roi = plt.imread(img_roi_path)
except:
    print("Exception at index ",index,". (Reading image)")

# ax_img.clear()
# ax_img.axis("off")
# ax_img.imshow(img_extr)
# title = str(index) + ": " + str(row["img_extr"])
# ax_img.title.set_text(title)

# ax_roi.clear()
# ax_roi.axis("off")
# ax_roi.imshow(img_roi)
# ax_roi.title.set_text(row["img_overlay"])
update_fig()
...
# ax.imshow(img_set_mean, cmap = 'gray',vmin=0, vmax=255) 
# ax.title.set_text(title), ax[row,col].axis("off")
...
# text_box_iter = mpl.widgets.TextBox(ax_textbox, "Iter","1")
# text_box_iter.on_text_change(txt_change)

# but_iter_x_value = 1 ##########
# but_iter_x =  mpl.widgets.Button(ax_iter_x,  "Iter({})".format(str(but_iter_x_value)) )

# but_iter_10 = mpl.widgets.Button(ax_iter_10, "Iter(10)")
# but_iter_20 = mpl.widgets.Button(ax_iter_20, "Iter(20)")
# but_iter_x.on_clicked(on_click_iter_x)
# but_iter_10.on_clicked(on_click_iter_10)
# but_iter_20.on_clicked(on_click_iter_20)
# but_iter_x.color = 'cyan'
# but_iter_10.color = 'cyan'
# but_iter_20.color = 'cyan'
