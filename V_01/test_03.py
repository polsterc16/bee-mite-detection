...
for img in list_diff:
    ...
    _,th = cv2.threshold(img, 31, 255, cv2.THRESH_BINARY)
...
for item in list_sobel:
    ...
    ax[row,i].title.set_text("sobel {}".format(i)), ax[row,i].set_xticks([]), ax[row,i].set_yticks([])
...
for item in list_thres:
    i+=1
    ax[row,i].imshow(item, cmap = 'gray',vmin=0, vmax=255) 
    ax[row,i].title.set_text("thres {}".format(i)), ax[row,i].set_xticks([]), ax[row,i].set_yticks([])
